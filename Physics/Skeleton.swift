...
private struct Joint {
  let i: Int
  let aIsA: Bool  // endpoint on bone i  (true = pA, false = pB)
  let j: Int
  let bIsA: Bool  // endpoint on bone j
}
...
// Named indices for bones in the two-legged skeleton
enum BoneID: Int, CaseIterable {
  case thighL, shinL, thighR, shinR, spine, neck
  case clavicleL, clavicleR, upperArmL, forearmL
  case upperArmR, forearmR, pelvis
}

// Named indices for muscles in the default setup
enum MuscleID: Int, CaseIterable {
  case quadL, hamL, quadR, hamR, flexL, extL, flexR, extR
}
...
final class Skeleton {
  ...
  subscript(_ id: BoneID) -> Bone {
    get { bones[id.rawValue] }
    set { bones[id.rawValue] = newValue }
  }

  subscript(_ id: MuscleID) -> Muscle {
    get { muscles[id.rawValue] }
    set { muscles[id.rawValue] = newValue }
  }
  ...
  static func twoLegs() -> Skeleton {
    ...
    let pelvisBone = makeBone(
      a: pelvis + SIMD2<Float>(-hipOffset, 0),
    ...
    let clavicleL = makeBone(
      a: chest,
      b: chest + SIMD2<Float>(-shoulderOffset, 0))
    let clavicleR = makeBone(
      a: chest,
      b: chest + SIMD2<Float>(shoulderOffset, 0))
    ...
    let upperArmL = makeBone(
      a: clavicleL.pB,
      b: clavicleL.pB + upperArmLDir)
    let forearmL = makeBone(
      a: upperArmL.pB,
      b: upperArmL.pB + upperArmLDir * 0.8)

    let upperArmR = makeBone(
      a: clavicleR.pB,
      b: clavicleR.pB + upperArmRDir)
    let forearmR = makeBone(
      a: upperArmR.pB,
  ...
  func step(
    ...
    for _ in 0..<Config.solverIterations {  // more iterations → better stability
      ...
      for jnt in joints {
        ...
        if anchoredRoots.contains(jnt.i) && jnt.aIsA || anchoredRoots.contains(jnt.j) && jnt.bIsA {
          centre = anchoredRoots.contains(jnt.i) && jnt.aIsA ? pa : pb
        }

        let deltaA = centre - pa
        let deltaB = centre - pb
        if !deltaA.x.isFinite || !deltaA.y.isFinite || !deltaB.x.isFinite || !deltaB.y.isFinite {
          continue
        }
...
// MARK: - Utility calculations
extension Skeleton {
  /// Approximate center of mass using average of all joint positions.
  func centerOfMass() -> SIMD2<Float> {
  ...
  /// Bounding box encompassing all joints.
  func boundingBox() -> (min: SIMD2<Float>, max: SIMD2<Float>) {
    guard var minP = bones.first?.pA else { return (.zero, .zero) }
    var maxP = minP
    for b in bones {
      minP = min(minP, b.pA)
      minP = min(minP, b.pB)
  ...
  /// Height of the skeleton in metres.
  func height() -> Float {
    let bb = boundingBox()
    return bb.max.y - bb.min.y
  }

  /// Returns true if both feet are in contact with the ground within a tolerance.
  func isGrounded(groundY: Float, tolerance: Float = 0.01) -> Bool {
    let footL = bones[BoneID.shinL.rawValue].pB
    let footR = bones[BoneID.shinR.rawValue].pB
    return abs(footL.y - groundY) <= tolerance && abs(footR.y - groundY) <= tolerance
  }
}
extension Skeleton {

  /// Uniformly scale all joint positions and rest lengths.
  mutating func scale(by factor: Float) {
    for i in bones.indices {
      bones[i].pA *= factor
      bones[i].pB *= factor
      bones[i].prevPA *= factor
      bones[i].prevPB *= factor
      bones[i].rest *= factor
    }
    for i in muscles.indices {
      muscles[i].rest *= factor
    }
  }

  /// Human-readable description of the skeleton state.
  func summary() -> String {
    let bb = boundingBox()
    return "bones: \(bones.count), muscles: \(muscles.count), height: \(bb.max.y - bb.min.y)"
  }
}
